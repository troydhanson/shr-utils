 ...
 *  shr-npub
 *
 *  reads frames from an shr ring and publishes them over nanomsg push socket
 *
 */
...
void usage() {
  ...
  fprintf(stderr,"options:\n"
                 "   -e <addr>  (remote receiver e.g. tcp://192.168.1.2:3456)\n"
...
int handle_io(void) {
  ...
  if (rv > 0) {
    ...
    /* push frames to nanomsg */
    for (i = 0; i < iovcnt; i++) {
      rc = nn_send(cfg.sock, cfg.iov[i].iov_base, cfg.iov[i].iov_len, 0);
      if (rc < 0) {
...
int main(int argc, char *argv[]) {
  ...
  /* open nanomsg push socket to peer */
  cfg.sock = nn_socket(AF_SP, NN_PUSH);
  if (cfg.sock < 0) goto done;
  cfg.eid = nn_connect(cfg.sock, cfg.endpoint);
